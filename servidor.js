  ...
  for (const [sportKey, eventosGrupo] of Object.entries(grupos)) {
    ...
    for (const evento of eventosGrupo) {
      for (const game of juegos) {
        ...
        const scoreDirecto = (calcularPuntuacionSimilitud(evento.local, evento.visitante, homeApi, awayApi, evento.horaInicio, game.commence_time, evento.sport, sportKey));
        const scoreCruzado = (calcularPuntuacionSimilitud(evento.local, evento.visitante, awayApi, homeApi, evento.horaInicio, game.commence_time, evento.sport, sportKey));
        const esCruzado = scoreCruzado > scoreDirecto;

        if (esCruzado) {
          evento.cuota_local = outcomes.find(o => o.name === awayApi)?.price || evento.cuota_local;
          evento.cuota_visitante = outcomes.find(o => o.name === homeApi)?.price || evento.cuota_visitante;
        } else {
          evento.cuota_local = outcomes.find(o => o.name === homeApi)?.price || evento.cuota_local;
          evento.cuota_visitante = outcomes.find(o => o.name === awayApi)?.price || evento.cuota_visitante;
        }
